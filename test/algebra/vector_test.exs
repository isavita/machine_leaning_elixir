...
defmodule Algebra.VectorTest do
  ...
  describe "Vector.multiply_scalar/2" do
    ...
    test "raises error when the second argument is not a scalar" do
      assert_raise ArgumentError, fn -> Vector.multiply_scalar([1, 2], [1, 2]) end
      assert_raise ArgumentError, fn -> Vector.multiply_scalar([1, 2], {1}) end
      assert_raise ArgumentError, fn -> Vector.multiply_scalar([1, 2], %{n: 1}) end
    end

    test "raises error when first argument is not a vector" do
      assert_raise ArgumentError, fn -> Vector.multiply_scalar(1, 2) end
      assert_raise ArgumentError, fn -> Vector.multiply_scalar(%{}, 2) end
    end
  end
